...
int EmulNet::ENsend(Address *myaddr, Address *toaddr, char *data, int size) {
	...
	#ifdef DEBUGLOG
		sprintf(temp, "%d.%d.%d.%d:%d Sending 4+%d B msg type %d to %d.%d.%d.%d:%d at time :%d",
		        myaddr->addr[0],myaddr->addr[1],myaddr->addr[2],myaddr->addr[3],*(short*)&myaddr->addr[4],size-4,
		        *(int *)data, toaddr->addr[0], toaddr->addr[1], toaddr->addr[2], toaddr->addr[3], *(short *)&toaddr->addr[4],par->getcurrtime());
		printf("%s\n", temp);
	#endif
